...
class _DashboardTabState extends State<DashboardTab> {
          ...
          lineBarsData: [
            LineChartBarData(
              ...
              dotData: FlDotData(show: true, getDotPainter: (spot, percent, barData, index) {
                return FlDotCirclePainter(radius: 3, color: lineColor, strokeWidth: 1, strokeColor: Colors.white);
              }),
            ...
            if (_fanLoading)
              ...
              Row(
                ...
                children: [
                  ...
                  const SizedBox(width: 8),
                  Expanded(child: _buildFanButton(text: 'AUTO', color: Colors.blue, action: 'AUTO', isOnline: isOnline, icon: Icons.hdr_auto)), // TOMBOL AUTO
                  const SizedBox(width: 8),
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        ...
        style: ElevatedButton.styleFrom(
          backgroundColor: isOnline ? color.withOpacity(0.2) : color.withOpacity(0.1),
          foregroundColor: color,
          elevation: 0,
          padding: EdgeInsets.zero,
          side: BorderSide(color: isOnline ? color : color.withOpacity(0.3)),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    GridView.count(
                      ...
                      mainAxisSpacing: 12,
                      children: [
                        _buildSensorCard(icon: FontAwesomeIcons.droplet, title: 'Humidity', value: isOnline ? (latestData?['hum'] != null ? '${latestData!['hum']}%' : '--') : '--', minMax: isOnline ? 'Min: 85.1%  Max: 85.3%' : 'Device Offline', color: Colors.blue, isOnline: isOnline),
                        _buildSensorCard(icon: FontAwesomeIcons.weightHanging, title: 'Pressure', value: isOnline ? (latestData?['pres'] != null ? '${latestData!['pres']} hPa' : '--') : '--', minMax: isOnline ? '995.8 hPa - 100.7 hPa' : 'Device Offline', color: Colors.green, isOnline: isOnline),
                        _buildSensorCard(icon: FontAwesomeIcons.temperatureHalf, title: 'Temperature', value: isOnline ? (latestData?['temp'] != null ? '${latestData!['temp']}°C' : '--') : '--', minMax: isOnline ? 'Avg: 29.3°C' : 'Device Offline', color: Colors.orange, isOnline: isOnline),
  ...
  Widget _buildHeader(bool isOnline) {
    ...
    String statusReason = isOnline ? 'Updated: ${sensorProvider.lastSeenText}' : 'Last: ${sensorProvider.lastSeenText}';
      ...
      child: Row(
        ...
        children: [
          ...
          Container(
            ...
            child: Row(children: [Icon(isOnline ? Icons.wifi : Icons.wifi_off, size: 16, color: statusColor), const SizedBox(width: 6), Text(statusText, style: TextStyle(color: statusColor, fontSize: 12, fontWeight: FontWeight.bold))]),
          ),
        ],
      ),
    );
  ...
  Widget _buildLightIntensityCard(dynamic luxValue, bool isOnline) {
    return Opacity(
      ...
      child: Container(
        decoration: BoxDecoration(
          gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Colors.amber.withOpacity(0.15), Colors.orange.withOpacity(0.1)]),
          borderRadius: BorderRadius.circular(16),
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            if (isOnline)
              Consumer<SensorProvider>(builder: (context, sensorProvider, child) {
                  final recentLightData = sensorProvider.trendData.where((data) => data['lux'] != null).take(10).map((data) => data['lux'].toString()).toList();
                  return Wrap(spacing: 8, children: recentLightData.map((value) => _buildDataPoint(value)).toList());
              })
            else
              const Text('Device Offline - No recent data', style: TextStyle(color: Colors.white54, fontSize: 12)),
          ],
        ),
      ),
    );
  ...
  Widget _buildDataPoint(String value) {
    return Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: Colors.white.withOpacity(0.1), borderRadius: BorderRadius.circular(6)), child: Text(value, style: const TextStyle(color: Colors.white70, fontSize: 12)));
  }
